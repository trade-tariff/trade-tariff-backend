...
namespace :search_analytics do # rubocop:disable Metrics/BlockLength
  ...
  task :validate_cloudwatch_queries do # rubocop:disable Rails/RakeEnvironment
    %w[period cloudwatch_snapshot_query journey_queries latency_histogram frontend_events_query journey_outcomes_query daily_query cloudwatch_query_validator].each do |service|
