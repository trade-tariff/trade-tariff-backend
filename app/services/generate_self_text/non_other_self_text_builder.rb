  ...
  class NonOtherSelfTextBuilder
    ...
    def segment_to_json(segment)
...

      {
        ...
        eu_self_text: node[:eu_self_text],
    ...
    def build_input_context(segment)
      ...
      context['eu_self_text'] = eu_self_text if eu_self_text
    ...
    def system_prompt
      @system_prompt ||= begin
        prompt = AdminConfiguration.classification.by_name('non_other_self_text_context')&.value
        raise 'non_other_self_text_context admin configuration not found - run rake admin_configurations:seed' unless prompt
    ...
    def model
      @model ||= AdminConfiguration.option_value('non_other_self_text_model')
    end

    def batch_size
      @batch_size ||= AdminConfiguration.integer_value('non_other_self_text_batch_size')
    end
  end
end
