...
module SearchAnalytics
  class ReadModelProjection
    ...
    def initialize(model:, dates:, period:, costs:)
      ...
      sql = <<~SQL
        WITH chosen AS MATERIALIZED (
          SELECT * FROM search_analytics_multi_day_observations WHERE read_model_id = #{@model_id} AND reporting_date IN (#{@dates})
          ...
          SELECT journey_key, bit_or(all_hours)>0 AS all_seen,
            ...
            search_analytics_latest_state(terminal) AS terminal, bit_or(flags) AS flags
          ...
          FROM search_analytics_journey_rollups WHERE read_model_id = #{@model_id} AND reporting_date IN (#{@dates}) AND bucket_size='day' GROUP BY view
          UNION ALL
          SELECT view, bucket, #{(%w[journeys] + SERIES).join(',')}
          FROM search_analytics_journey_rollups WHERE read_model_id = #{@model_id} AND reporting_date IN (#{@dates}) AND bucket_size='#{period.bucket_size}'
    ...
    def terms
      ...
      @db.fetch(<<~SQL).all.map { |row| row.transform_keys(&:to_s) }
        WITH totals AS (
          ...
          WHERE r.service=#{@db.literal(@service)} AND r.reporting_date IN (#{@dates})
    ...
    def cost_keys
      ...
      @db.fetch("SELECT DISTINCT encode(journey_key,'hex') AS key FROM search_analytics_journey_days WHERE read_model_id = #{@model_id} AND reporting_date IN (#{@dates}) AND #{column}>0 AND journey_key IN (#{keys})").all.to_h { |row| [row[:key], true] }
    end
