  ...
  class JourneyOutcomesQuery
    def self.call(source:, log_stream_filter:)
      terminal = <<~SQL.squish
        ...
        (search_type = 'classic' OR ((search_type = 'interactive' OR search_type = 'internal') AND
      ...
      <<~SQL
        ...
        FROM (
          SELECT request_id, COUNT(*) AS observed_events,
            ...
            MAX(CASE WHEN #{terminal} AND result_count = 0 AND (search_degraded IS NULL OR search_degraded = false) THEN 1 ELSE 0 END) AS zero_result
          FROM #{source}
