module SearchDiagnostics
  ...
  private
    ...
    def cloudwatch_query
      <<~QUERY
        fields @timestamp, @message, event, request_id, search_type, trace_version, query, base_query, effective_query, original_query, expanded_query, refined_query, added_answers, reason, result_count, chapter_result_count, heading_result_count, commodity_result_count, other_result_count, max_score, total_duration_ms, duration_ms, leg, status, retrieval_method, stage, iteration, match_source, matched_value, target_type, target_id, target_endpoint, goods_nomenclature_item_id, goods_nomenclature_sid, goods_nomenclature_class, provider, model, event_kind, input_tokens, cached_input_tokens, cache_write_input_tokens, output_tokens, reasoning_tokens, total_tokens, input_cost_usd, cached_input_cost_usd, cache_write_input_cost_usd, output_cost_usd, total_cost_usd, pricing_known, finish_reason, served_model, service_tier, reasoning_effort, openai_request_id, openai_response_id, response_type, attempt_number, question_count, logged_question_count, questions_truncated, answer_count, candidate_count, logged_candidate_count, candidates_truncated, ranked_answer_count, logged_ranked_answer_count, ranked_answers_truncated, confidence_levels, ranking_source, note_evidence_enabled, note_evidence_status, considered_note_count, considered_evidence_count, considered_association_count, considered_distinct_source_count, selected_note_count, selected_evidence_count, selected_distinct_source_count, omitted_evidence_count, logged_omitted_evidence_count, omitted_evidence_truncated, final_result_type, results_type, total_attempts, total_questions, result_limit, error_type, error_message, error_message_truncated, details, description_intercept_matched, description_intercept_term, description_intercept_excluded, description_intercept_filtering, description_intercept_filter_prefix_count, description_intercept_guidance_level, description_intercept_guidance_location, description_intercept_escalate_to_webchat, matched, term, excluded, filtering, filter_prefix_count, guidance_level, guidance_location, escalate_to_webchat
        | filter request_id = #{request_id.to_json} and (service = "search" or (service = "ai_usage" and event_kind = "vector_search_query_embedding"))
        | sort @timestamp asc
        | limit #{limit}
      QUERY
    end
