class EnquiryForm::Submission < Sequel::Model(Sequel[:enquiry_forms].qualify(:public))
  ...
  def create_reference_number(length = 8)
    loop do
      ...
      unless EnquiryForm::Submission.where(reference_number: reference_number).exists?
