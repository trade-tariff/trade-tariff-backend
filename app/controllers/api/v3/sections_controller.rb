module Api
  module V3
    class SectionsController < BaseController
      def index
        sections = Section.eager(:section_note, :customs_tariff_section_note).all
        render json: Api::V3::SectionSerializer.collection(sections)
      end

      def show
        section = Section.where(position: params[:id].to_i)
          .eager(:section_note, :customs_tariff_section_note)
          .take
      ...
      def chapters
        section = Section.where(position: params[:id].to_i)
          .eager(chapters: :goods_nomenclature_descriptions)
          .take
        raise Sequel::RecordNotFound, "Section #{params[:id]} not found" if section.nil?
