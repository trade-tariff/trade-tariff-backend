Sequel.migration do
  up do
    tunas = GoodsNomenclatureDescription.where(goods_nomenclature_description_period_sid: 154_585).first
    if tunas.present?
    ...
    engines = GoodsNomenclatureDescription.where(goods_nomenclature_description_period_sid: 157_423).first
    if engines.present?
