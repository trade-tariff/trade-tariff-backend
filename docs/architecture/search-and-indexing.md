...
Stored admin analytics exclude every event for a request ID with a recorded `search_degraded: true`, `search_failed`, or `search_stage_failed` search event in the selected time window. The snapshot failure lookup uses the same UK/XI log stream as its metrics. Missing, null, and empty request IDs remain included, as do historical requests without a linked failure. Use the complete journey window: a failure outside that window cannot exclude an event inside it. Search Overview does not use that cohort: it counts `TradeTariff/Search` metrics as each event is recorded and does not retract an earlier count when a later failure is linked. Operations also counts events through metrics. Quality reuses existing metrics for its completed-search, selection and empty-result totals, with the same counting rules and combined UK/XI grouping. Its other widgets, Experiment and general AI Costs retain their existing log cohorts.
