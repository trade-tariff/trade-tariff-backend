RSpec.describe ErrorsController do
  ...
  shared_examples 'a csv or json error response' do |status_code, message|
    context 'with json request' do
      let(:make_request) { get "/uk/api/#{status_code}.json", headers: { 'HTTP_ACCEPT' => 'application/vnd.hmrc.2.0+json' } }
    ...
    context 'with other request' do
      let(:make_request) { get "/uk/api/#{status_code}.pdf", headers: { 'HTTP_ACCEPT' => 'application/vnd.hmrc.2.0+pdf' } }
