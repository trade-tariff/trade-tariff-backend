RSpec.describe 'Stored search analytics materialized views', :truncation do
  ...
  it 'serves the existing API contract from the current materialized views' do
  ...
  it 'serves fresh source data rather than stale materialized views after replacement' do
    ...
    expect(SearchAnalytics::MaterializedProjection).not_to receive(:new)
    get '/uk/admin/search_analytics.json', params: { period: '24h', view: 'all' }, headers: request_headers(format: :json)
    expect(response).to have_http_status(:ok)
    expect(JSON.parse(response.body).dig('data', 'attributes', 'summary', 'searches')).to eq(2)
  end

  it 'retains date validation before accessing materialized views' do
    expect(SearchAnalytics::MaterializedProjection).not_to receive(:new)
