...
RSpec.describe 'search overview dashboard Terraform' do
  ...
  it 'reads TradeTariff/Search metrics instead of scanning logs' do
    expect(dashboard).to include('TradeTariff/Search')
    expect(dashboard).to include('type   = "metric"')
    ...
    Search::Metrics::METRIC_NAMES.each do |name|
      expect(dashboard).to include(name)
    end
  end
