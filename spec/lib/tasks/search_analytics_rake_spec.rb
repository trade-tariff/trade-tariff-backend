RSpec.describe 'search analytics rake tasks' do
  ...
  describe 'search_analytics:validate_cloudwatch_queries' do
    ...
    %w[uk xi].each do |service|
      it "loads the daily SQL dependencies for #{service} without a database" do
        Tempfile.create(['analytics-validator-client', '.rb']) do |file|
          ...
          expect(output).to include('Validated daily/search_journeys', 'Validated daily/journey_outcomes', "Validated #{service == 'uk' ? 10 : 9} distinct CloudWatch queries")
