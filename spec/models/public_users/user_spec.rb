RSpec.describe PublicUsers::User do
  ...
  describe '#chapter_ids=' do
    let(:ids) { '12,34,56' }

    it 'updates the associated model' do
      user.chapter_ids = ids
      expect(user.preferences.chapter_ids).to eq ids
    end

    it 'delegates chapter_id to the preferences association' do
      user.chapter_ids = ids
      expect(user.chapter_ids).to eq ids
    end
  end
end
