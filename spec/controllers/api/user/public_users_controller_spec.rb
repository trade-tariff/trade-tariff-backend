RSpec.describe Api::User::PublicUsersController do
  ...
  describe 'POST #destroy' do
    ...
    context 'when user exists and is soft deleted' do
      ...
      it 'deactivates the stop_press_subscription' do
        make_request
        expect(user.reload.stop_press_subscription).to be false
      end
    end

    context 'when user exists and is already soft deleted' do
      ...
      it 'does not change the deleted flag' do
        expect {
          make_request
        }.not_to change { user.reload.deleted }.from(true)
      end

      it 'does not reactivate or deactivate stop_press_subscription' do
        original_state = user.stop_press_subscription
        make_request
        expect(user.reload.stop_press_subscription).to eq original_state
      end
    end
  end
end
