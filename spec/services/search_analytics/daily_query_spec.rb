RSpec.describe SearchAnalytics::DailyQuery do
  ...
  it 'reruns only the missing query after a later query fails' do
    client.stub_responses(:get_query_results, [*Array.new(8) { complete }, complete.merge(status: 'Failed')])
  ...
  it 'hashes cost request IDs before storing them and normalises operations' do
    ...
    client.stub_responses(:get_query_results, [complete, complete, complete.merge(results: [row]), complete.merge(results: [row]), *Array.new(12) { complete }])
  ...
  it 'hashes complete frontend journey sets before storing them' do
    ...
    client.stub_responses(:get_query_results, [*Array.new(8) { complete }, response, *Array.new(7) { complete }])
  ...
  it 'splits incomplete journey output even when below the row cap' do
    client.stub_responses(:get_query_results, [*Array.new(8) { complete }, complete.merge(statistics: { records_matched: 2.0 }), *Array.new(9) { complete }])
    collect
    expect(starts.size).to eq(18)
    expect(starts[9][:params][:query_string]).to include('2026-09-14 01:30:00')
    expect(starts[10][:params][:query_string]).to include('2026-09-14 01:30:00')
  end

  it 'fails safely without matched-event statistics for journey completeness' do
    client.stub_responses(:get_query_results, [*Array.new(8) { complete }, complete.merge(statistics: {})])
