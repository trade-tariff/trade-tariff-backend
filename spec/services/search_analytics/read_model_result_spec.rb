RSpec.describe SearchAnalytics::ReadModelResult, :truncation do
  ...
  def rebuild
    SearchAnalytics::ReadModelRefresh.call(**scope, from: first_date, to: first_date + 1)
  end
  ...
  context 'with a current generation' do
    before { rebuild }
    ...
    it 'pins source rows and the generation while another connection replaces it' do
      expected = SearchAnalytics::DailyResults.legacy_call(**arguments)
      ready = Queue.new
      release = Queue.new
      pause = true
      allow(SearchAnalytics::ReadModelProjection).to receive(:new).and_wrap_original do |method, **args|
        if pause
          pause = false
          ready << true
          release.pop
    ...
    it 'does not load identifier or term blobs into Ruby' do
      expect(SearchAnalytics::ReadModelAggregate).to receive(:new).and_wrap_original do |method, **args|
  ...
  context 'with incompatible derived data' do
    ...
    it 'does not combine a new day with an old generation' do
      store_day(first_date - 1)
      expect(described_class.call(**arguments(from: first_date - 1)).available).to be(false)
    end

    it 'rejects an older read-model processing version' do
      SearchAnalyticsReadModel.dataset.update(version: 0)
      expect(described_class.call(**arguments).available).to be(false)
    end
  end
