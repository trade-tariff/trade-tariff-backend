locals {
  ...
  db_replicate_command = ["/bin/sh", "-c", "./config/db_replicate.sh"]
  ...
  db_replicate_uk_secret_value = try(data.aws_secretsmanager_secret_version.db_replicate_uk_worker_configuration.secret_string, "{}")
  db_replicate_uk_secret_map   = jsondecode(local.db_replicate_uk_secret_value)
  db_replicate_uk_secret_env_vars = [
    for key, value in local.db_replicate_uk_secret_map : {
      name  = key
      value = value
    }
  ]
