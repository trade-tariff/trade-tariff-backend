...
- This repo uses `.envrc` for the Ruby/Bundler and worktree environment. In non-interactive agent shells, run project commands through `direnv exec <repo-path> <command>` so hooks and tools see the same environment as an interactive shell.
- Do not run `bundle install`, `bundle exec`, `bin/*`, `rspec`, `rubocop`, `git commit`, or `git push` directly from an agent shell when `.envrc` exists. Use `direnv exec <repo-path> ...`; if a direct command fails with missing gems or executables, treat it as a harness failure and retry with `direnv exec`.
