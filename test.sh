...
job_id=$(curl -s -X POST "http://localhost:3000/bulk_search" -d @body.json -H 'Content-Type: application/json' -H 'Accept: application/json' | jq -r '.data.id')

echo "Job ID: $job_id"
# Continually poll the status of the job until it's completed
while true; do
    # GET request to check the status of the job
    response=$(curl -s "http://localhost:3000/bulk_search/${job_id}" -H 'Content-Type: application/json' -H 'Accept: application/json')
    ...
    if [ "$status" = "completed" ] || [ "$status" = "error" ] || [ "$status" = "not_found" ]; then
        ...
        echo "$response" | jq -r '.included | .[] | select(.type == "search_result_ancestor")'
        break
    else
